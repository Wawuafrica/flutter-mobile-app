...
void main() async {
  ...
  try {
    ...
    _logger.e('Main: Error loading .env file: $e. Ensure .env file exists and is accessible.');
    // Consider showing a fatal error screen here if env vars are critical
    runApp(
      const MaterialApp(
        home: Scaffold(
          body: Center(
            child: Text('Fatal Error: Failed to load configuration. Please contact support.'),
          ),
        ),
      ),
    );
  ...
  try {
    ...
    await apiService.initialize(
      apiBaseUrl: dotenv.env['API_BASE_URL'] ?? 'https://staging.wawuafrica.com/api',
    ...
    _logger.i('Main: PusherService initialized successfully and connection attempted.');
    ...
    runApp(
      MultiProvider(
        providers: [
          ...
          Provider<PusherService>.value(value: pusherService), // Provide PusherService
    ...
    _logger.e('Main: Fatal error during app initialization. Showing fallback UI.', error: e, stackTrace: st);
...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  Future<void> _updateConnectionStatus(List<ConnectivityResult> result) async {
    final bool hadConnection = !_connectionStatus.contains(ConnectivityResult.none);
    ...
    _logger.d('MyApp: Connectivity status updated. Current: $_connectionStatus');

    if (!hadConnection && hasConnectionNow) {
      _logger.i('MyApp: Network just became available. Attempting to re-engage services...');
      ...
      if (widget.pusherService.isInitialized) {
        _logger.d('MyApp: PusherService is initialized, calling resubscribeToChannels.');
        widget.pusherService.resubscribeToChannels();
      } else {
        _logger.w('MyApp: PusherService is not initialized after network came back. This might indicate an earlier failure.');
        ...
        try {
          ...
          _logger.i('MyApp: PusherService successfully re-initialized after network recovery.');
        } catch (e) {
          _logger.e('MyApp: Failed to re-initialize PusherService on network recovery: $e');
        }
      }
  ...
  @override
  void dispose() {
    _logger.d('MyApp: Disposing of connectivity subscription and WidgetsBindingObserver.');
